...
def test_race_without_proficiencies():
    response = client.get("/racas/5")  # Draconato não tem proficiências
    assert response.status_code == 200
    data = response.json()
...
def test_race_optional_fields_absent():
    response = client.get("/racas/4")  # Humano não tem visao_no_escuro
    assert response.status_code == 200
    data = response.json()
...
def test_search_with_accent():
    response = client.get("/racas?name=anao")  # sem acento
    assert response.status_code == 200
    data = response.json()
...
def test_combined_filter_case_accent():
    response = client.get("/racas?name=ELFO&size=MEDIO")
    assert response.status_code == 200
    data = response.json()
    assert all("elfo" in r["nome"].lower() and "médio" in r["tamanho"].lower() for r in data)

def test_combined_filters():
    response = client.get("/racas?name=elfo&size=médio&bonus=destreza")
    assert response.status_code == 200
    data = response.json()
    assert all(
...
def test_subrace_fields():
    response = client.get("/racas/2")  # Elfo
    assert response.status_code == 200
    data = response.json()
...
def test_subrace_characteristics_content():
    response = client.get("/racas/1")  # Anão
    assert response.status_code == 200
    data = response.json()
...
def test_get_subraces_of_race():
    response = client.get("/racas/2/subracas")  # Elfo
    assert response.status_code == 200
    data = response.json()
    assert isinstance(data, list)
    assert len(data) > 0
...
def test_get_subrace_by_id():
    response = client.get("/subracas/2_1")  # Alto Elfo
    assert response.status_code == 200
    data = response.json()
...
def test_search_subraces_by_name():
    response = client.get("/subracas?name=floresta")
    assert response.status_code == 200
    data = response.json()
    assert any("floresta" in sub["nome"].lower() for sub in data)

def test_order_by_nome():
    response = client.get("/racas?order=nome")
    assert response.status_code == 200
    data = response.json()
...
def test_filter_by_visao_no_escuro():
    response = client.get("/racas?filter=visao_no_escuro")
    assert response.status_code == 200
    data = response.json()
...
def test_filter_by_bonus_forca():
    response = client.get("/racas?bonus=forca")
    assert response.status_code == 200
    data = response.json()
